...
namespace Ilargi
{
	...
	void VulkanSwapchain::Present()
	{
		...
		Renderer::NewFrame(currentFrame);
	}
	...
	void VulkanSwapchain::RecordCommandBuffer(VkCommandBuffer cmdBuffer, uint32_t imageIndex)
	{
		...
		//VkCommandBuffer cb = std::static_pointer_cast<VulkanCommandBuffer>(commandBuffer)->GetCurrentCommand(Renderer::GetCurrentFrame());
		...
		//std::static_pointer_cast<VulkanPipeline>(pip)->Bind(cb, framebuffers[imageIndex], extent.width, extent.height);
		//
		//std::static_pointer_cast<VulkanVertexBuffer>(vertexBuffer)->Bind(cb);
		//std::static_pointer_cast<VulkanIndexBuffer>(indexBuffer)->Bind(cb);
		//
