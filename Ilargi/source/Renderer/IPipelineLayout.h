...
namespace Ilargi
{
	...
	class IPipelineLayout : public std::enable_shared_from_this<IPipelineLayout>
	{
	public:
		/*
		...
		*/
		...
		std::shared_ptr<T> As()
		{
			ILG_STATIC_ASSERT(std::is_base_of<IPipelineLayout, T>::value, "T must be a derived class of PipelineLayout");
		...
		*/
		static std::shared_ptr<IPipelineLayout> Create(const PipelineLayoutProperties& aProperties);
	};
}
