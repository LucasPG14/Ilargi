...
namespace Ilargi
{
	...
	public:
		...
		*/
		...
		std::shared_ptr<T> As()
		{
			ILG_STATIC_ASSERT(std::is_base_of<IFramebuffer, T>::value, "T must be a derived class of Framebuffer");
		...
		*/
		static std::shared_ptr<IFramebuffer> Create(const FramebufferProperties& aProperties);
	};
}