...
namespace Ilargi
{
	...
	class IDescriptorSetLayout : public std::enable_shared_from_this<IDescriptorSetLayout>
	{
	public:
		/*
		...
		*/
		...
		std::shared_ptr<T> As()
		{
			ILG_STATIC_ASSERT(std::is_base_of<IDescriptorSetLayout, T>::value, "T must be a derived class of DescriptorSetLayout");
		...
		*/
		static std::shared_ptr<IDescriptorSetLayout> Create(const DescriptorSetLayoutProperties& aProperties);
	};
}
