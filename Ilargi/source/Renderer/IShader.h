...
namespace Ilargi
{
	...
	public:
		...
		*/
		...
		/*
		* @brief Returns the amount of vertex inputs of the shader(only for vertex shader).
		* @return The amount of vertex inputs.
		*/
		virtual const uint32_t GetVertexInputsCount() const = 0;
		...
		*/
		...
		std::shared_ptr<T> As()
		{
			ILG_STATIC_ASSERT(std::is_base_of<IShader, T>::value, "T must be a derived class of IShader");
		...
		*/
		static std::shared_ptr<IShader> Create(std::string_view aFilepath);
	};
	...
	public:
		...
		*/
		void Add(std::string aName, const std::shared_ptr<IShader>& aShader);
		...
		*/
		[[nodiscard]] const std::shared_ptr<IShader>& Get(std::string aName);

	private:
		std::unordered_map<std::string, std::shared_ptr<IShader>> mShaders; // The container of the shaders.
	};
}