...
static void ImGui_ImplVulkan_CreateShaderModules(VkDevice device, const VkAllocationCallbacks* allocator)
{
    ...
    {
        std::vector<char> fragCode = readFile("shaders/imgui/vert.spv");

        VkShaderModuleCreateInfo vert_info = {};
        vert_info.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
        vert_info.codeSize = fragCode.size();
        vert_info.pCode = reinterpret_cast<const uint32_t*>(fragCode.data());
        VkResult err = vkCreateShaderModule(device, &vert_info, allocator, &bd->ShaderModuleVert);
        check_vk_result(err);
    }
    if (bd->ShaderModuleFrag == VK_NULL_HANDLE)
    {
        std::vector<char> fragCode = readFile("shaders/imgui/frag.spv");

        VkShaderModuleCreateInfo frag_info = {};
        frag_info.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
        frag_info.codeSize = fragCode.size();
        frag_info.pCode = reinterpret_cast<const uint32_t*>(fragCode.data());
